...
├── AutoVipProHook.java        # 【F】自动盲扫: 方法名/类名强词 + 字段按原值类别 + 单例实例字段注入 + v1.8结构盲扫(内存对象构造器签名)
